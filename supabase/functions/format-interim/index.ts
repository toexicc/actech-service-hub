// AI Interim Report: new findings discovered mid-repair (Ongoing Service) that
// need the client's approval before work continues. Mirrors format-diagnosis,
// but takes the already-approved initial diagnosis as context and references it.
...
const OTHER_SECTION =
  /^(to proceed|summary|recommendations?|findings?|cause of issue|cause|suggested solutions?|solution|note|notes|disclaimer|warranty|reference)\b/i;
...
const enforceAmountPlaceholders = (text: string): string => {
  ...
  const out = lines.map((line, idx) => {
    ...
    let out = line;
    out = out.replace(/(?:php|₱|p)\s*\[?\{?\s*[\d,]+(?:\.\d{1,2})?\s*\}?\]?/gi, AMOUNT_PLACEHOLDER);
    ...
    if (!isOptionLine(line)) {
      ...
      if (nextBare && !OTHER_SECTION.test(nextBare) && isOptionLine(nextBare)) {
        return out.replace(/(?:Php\s*)?\{Enter Amount\}/g, "").replace(/[\s\-–:]+$/, "").trimEnd();
      }
    }
...
const breakdownServiceNames = (lines: string[]): string[] => {
  ...
  for (let i = idx + 1; i < lines.length; i++) {
    ...
    const name = bare.split(/\s+-\s*php/i)[0].replace(/[-:]\s*$/, "").trim();
...
serve(async (req) => {
  ...
  try {
    const { initialDiagnosis, interimFindings, customerName, deviceType, model, serviceId } =
      await req.json();
    if (!interimFindings) {
      return new Response(JSON.stringify({ error: "Interim findings are required" }), {
...
You will write an INTERIM REPORT: new findings discovered while the repair was already ongoing, which the customer must approve before work continues. You are given the initial diagnosis (already approved) and the technician's new raw notes.
...
AC TECH INTERIM REPORT

Reference: <one or two sentences referring to the initial diagnosis and what was already approved>

Findings:
<clear explanation of the NEW findings discovered during the ongoing repair>

Cause of Issue:
<simple explanation of the root cause of the new findings>

Suggested Solution:
<the additional repair actions now needed>

Recommendations:
<professional advice for the customer>

Service Breakdown:
<Additional Service Item 1> - Php {Enter Amount}
<Additional Service Item 2 with part grade choices>
Option A - OEM: Php {Enter Amount}
Option B - Original: Php {Enter Amount}


Warranty:
<Additional Service Item 1> - {Enter Warranty Duration}


To proceed with the additional service, PROCEED or APPROVE to confirm your approval and kindly review our Terms and Conditions: bit.ly/actech-termsnconditions
Note: The quoted price excludes 12% VAT.

SUMMARY: <one-line summary of the additional repair needed>
...
CRITICAL SCOPE RULE: The Service Breakdown must list ONLY the ADDITIONAL services caused by the new findings. Never repeat, re-quote or restate services that were already part of the initial diagnosis.
...
When a service can be done with different part grades or variants, write the service name alone on its line, then list each variant as "Option A - <Variant>: Php {Enter Amount}".
Immediately after the Service Breakdown items output a "Warranty:" heading, then one line per additional service in the format "<Service Name> - {Enter Warranty Duration}". Never invent a warranty duration.
...
Initial diagnosis (already approved by the customer):
${initialDiagnosis || "(none on file)"}

Technician's new raw findings during the ongoing repair:
${interimFindings}

Produce the interim report now using the EXACT template. Do not include this instruction in your output.`;
    ...
    if (!raw) {
      return new Response(JSON.stringify({ error: "No interim report received" }), {
        status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    let formattedInterim = stripMarkdown(raw);
    formattedInterim = enforceAmountPlaceholders(formattedInterim);
    formattedInterim = enforceWarrantyLine(formattedInterim);

    return new Response(JSON.stringify({ formattedInterim }), {
