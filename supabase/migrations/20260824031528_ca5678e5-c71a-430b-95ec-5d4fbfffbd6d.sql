-- 1. Default service_date to the Manila calendar date at creation
ALTER TABLE public.services
  ALTER COLUMN service_date SET DEFAULT ((now() AT TIME ZONE 'Asia/Manila')::date);

-- 2. Backfill missing service dates from the received date (Manila calendar day)
UPDATE public.services
SET service_date = ((date_received AT TIME ZONE 'Asia/Manila')::date)
WHERE service_date IS NULL
  AND date_received IS NOT NULL;

-- 3. Make the atomic intake function set service_date explicitly
CREATE OR REPLACE FUNCTION public.create_service_atomic(_payload jsonb, _queue_id uuid DEFAULT NULL::uuid)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.services (
    ...
    auto_approve_diagnosis, rush_fee, service_date
  ) VALUES (
    v_service_id,
    ...
    COALESCE(nullif(_payload->>'service_date', '')::date, (now() AT TIME ZONE 'Asia/Manila')::date)
  );
