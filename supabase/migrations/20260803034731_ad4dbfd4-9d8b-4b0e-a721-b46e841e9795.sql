...
CREATE OR REPLACE FUNCTION public.prevent_service_id_change()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF NEW.service_id IS DISTINCT FROM OLD.service_id THEN
    RAISE EXCEPTION 'A ticket service ID cannot be changed after creation';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS services_prevent_service_id_change ON public.services;
CREATE TRIGGER services_prevent_service_id_change
BEFORE UPDATE OF service_id ON public.services
FOR EACH ROW EXECUTE FUNCTION public.prevent_service_id_change();