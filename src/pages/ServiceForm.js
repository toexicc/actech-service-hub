...
const buildFormSchema = (isPublic) => z.object({
...
const ServiceForm = ({ embeddedQueueId, embedded, onCompleted } = {}) => {
    const navigate = useNavigate();
    const location = useLocation();
    const isPublic = !embedded && location.pathname === "/intake";
    const searchQueueId = useMemo(() => new URLSearchParams(location.search).get("queueId"), [location.search]);
    ...
    const [kioskQr, setKioskQr] = useState(null);
    ...
    const adminStaffOptions = useMemo(() => staffData
        ...
        .sort((a, b) => {
        const rank = (role) => role?.trim().toLowerCase() === "admin" ? 0 : 1;
    ...
    useEffect(() => {
        if (loggedInUserRole === "admin" && loggedInUserFullName && adminList.length > 0) {
            ...
            const matchingAdmin = adminList.find(admin => admin.toLowerCase() === loggedInUserFullName.toLowerCase());
    ...
    const [prefilledQueueId, setPrefilledQueueId] = useState(null);
    useEffect(() => {
        if (!queueId || isPublic || prefilledQueueId === queueId)
            return;
        (async () => {
            ...
            if (!entry)
                return;
            const payload = (entry.form_payload || {});
            ...
            if (payload.ack1 && payload.ack2 && payload.ack3)
                setTermsRead(true);
            // Rehydrate captured images (stored as data URLs in the queue payload).
            if (payload.annotationImageUrl)
                setAnnotationImageUrl(payload.annotationImageUrl);
            ...
            if (entry.client_name)
                form.setValue("clientName", entry.client_name);
            if (entry.contact_number)
                form.setValue("phone", entry.contact_number);
            if (entry.device_type)
                form.setValue("deviceType", entry.device_type);
            if (entry.brand)
                form.setValue("brand", entry.brand);
            if (entry.model)
                form.setValue("model", entry.model);
            if (entry.chief_complaint)
                form.setValue("chiefComplaint", entry.chief_complaint);
    ...
    useEffect(() => {
        ...
        QRCode.toDataURL(`https://actechrepair-service.com/queue?entry=${encodeURIComponent(kioskCode)}`, { width: 420, margin: 1 })
            .then(setKioskQr)
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            if (isPublic)
                formData.append("Source", "Public Intake");
            ...
            const sanitizeFileName = (str) => str.replace(/[^a-zA-Z0-9]/g, '_');
            ...
            const techDepartmentsArr = [...new Set(techNamesArr.map(n => technicianList.find(t => t.name === n)?.department).filter(Boolean))];
            ...
            if (upsertError) {
                if (upsertError.code === "23505") {
                    throw new Error(`Service ID ${finalServiceId} is already in use. Please submit the form again to get a new ID.`);
                }
            ...
            if (isResponseOk) {
                ...
                Promise.allSettled([
                    ...techNames.map(techName => notifyNewServiceAssignment({
                        serviceId: finalServiceId,
                        clientName: data.clientName,
                        technician: techName,
                        adminRep: data.adminRep || "",
    ...
    const content = (<div className="p-4 md:p-8 animate-fade-in pb-8">
        ...
        {!isPublic && (<div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
          <h2 className="text-lg font-semibold text-green-600 mb-3">Client ID Search</h2>
          <div className="flex gap-2">
            <Input placeholder="Enter Client ID to load client information" value={searchClientId} onChange={(e) => setSearchClientId(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearchClientId()} className="flex-1"/>
            <Button type="button" onClick={handleSearchClientId} disabled={isSearchingClient} className="bg-green-600 hover:bg-green-700">
              <Search className="mr-2 h-4 w-4"/>
              {isSearchingClient ? "Searching..." : "Search"}
            </Button>
          </div>
          {form.watch("clientId") && (<p className="mt-2 text-sm text-green-600 font-medium">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            {!isPublic && (<div className="grid md:grid-cols-2 gap-4">
              <FormField control={form.control} name="adminRep" render={({ field }) => (<FormItem>
                    <FormLabel>Admin Rep:</FormLabel>
                    <FormControl>
                      <MultiSelect options={adminStaffOptions} selected={field.value ? field.value.split(", ").filter(Boolean) : []} onChange={(values) => field.onChange(values.join(", "))} placeholder="Select Admin(s)" grouped/>
                    </FormControl>
                    <FormMessage />
                  </FormItem>)}/>

              <FormField control={form.control} name="receivingStaff" render={({ field }) => (<FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        {receivingStaffOptions.length > 0 ? (["Admin", "Management"].map((groupName, index) => {
                    const options = receivingStaffOptions.filter((staff) => staff.group === groupName);
                    if (options.length === 0)
                        return null;
                    return (<SelectGroup key={groupName}>
                                {index > 0 && <SelectSeparator />}
                                <SelectLabel>{groupName}</SelectLabel>
                                {options.map((staff) => (<SelectItem key={staff.value} value={staff.value}>
                                    {staff.label}
                                  </SelectItem>))}
                              </SelectGroup>);
              ...
              <FormField control={form.control} name="technicianDepartments" render={({ field }) => {
                ...
                const deptOptions = Array.from(new Set(technicianList.map((t) => t.department).filter(Boolean)))
                    .filter((dept) => {
                    if (dept === SPECIAL_CASE_DEPARTMENT)
                        return true;
                    if (!deviceType)
                        return true;
                ...
                const preview = selectedDepts
                    ...
                    const pool = technicianList.filter((t) => t.department === dept);
                    if (pool.length === 0)
                        return `${dept}: (no active technicians)`;
                ...
                return (<FormItem>
                      ...
                      <FormControl>
                        <MultiSelect options={deptOptions} selected={selectedDepts} onChange={(values) => field.onChange(values.join(", "))} placeholder="Select Departments (auto-assigns a technician)"/>
                      </FormControl>
                      {preview && (<p className="mt-1 text-xs text-muted-foreground">
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {!isPublic && (<>
                    <FormField control={form.control} name="clientType" render={({ field }) => (<FormItem>
            ...
            const availableDeviceTypes = selectedTechDepartments.length > 0
                ? Array.from(new Set(selectedTechDepartments.flatMap(dept => dept === SPECIAL_CASE_DEPARTMENT
                    ? [...DEVICE_TYPES]
                    : DEVICE_TYPES_BY_DEPARTMENT[dept] || [])))
                : DEVICE_TYPES;
            return (<FormItem>
                        <FormLabel>Device Type:</FormLabel>
                        {showOtherDeviceInput ? (<FormControl>
                            <Input {...field} placeholder="Enter device type" onBlur={() => {
                        if (!field.value) {
                            setShowOtherDeviceInput(false);
                        }
                    }}/>
                          </FormControl>) : (<Select onValueChange={(value) => {
                        if (value === "Others") {
                            ...
                            <SelectContent>
                              {availableDeviceTypes.map((deviceType) => (<SelectItem key={deviceType} value={deviceType}>
                                  {deviceType}
                                </SelectItem>))}
                            </SelectContent>
                          </Select>)}
                ...
                <FormField control={form.control} name="serial" render={({ field }) => (<FormItem>
                ...
                <FormField control={form.control} name="memory" render={({ field }) => (<FormItem>
            ...
            {/* Chief Complaint */}
            <FormField control={form.control} name="chiefComplaint" render={({ field }) => (<FormItem>
                  <div className="flex items-center justify-between">
                    <FormLabel>Chief Complaint:</FormLabel>
                    {!isPublic && (<Button type="button" size="sm" variant="outline" disabled={isFormattingComplaint || !field.value?.trim()} onClick={async () => {
                    const raw = field.value?.trim();
                    if (!raw)
                        return;
                    setIsFormattingComplaint(true);
                    try {
                        const { data: resp, error } = await supabase.functions.invoke("format-complaint", { body: { rawComplaint: raw, mode: isPublic ? "brief" : "detailed" } });
                        if (error)
                            throw error;
                      ...
                      {isFormattingComplaint ? (<><Loader2 className="mr-2 h-3.5 w-3.5 animate-spin"/>Formatting…</>) : ("Format with AI")}
                    </Button>)}
                  </div>
                  <FormControl>
            ...
            {/* Device Password */}
            <FormField control={form.control} name="devicePassword" render={({ field }) => (<FormItem>
            ...
            <div>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <FormField control={form.control} name="dents" render={({ field }) => (<FormItem className="flex items-center space-x-2 space-y-0">
                      <FormControl>
                ...
                <FormField control={form.control} name="scratches" render={({ field }) => (<FormItem className="flex items-center space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={(checked) => {
                field.onChange(checked);
                if (checked) {
                ...
                <FormField control={form.control} name="missingParts" render={({ field }) => (<FormItem className="flex items-center space-x-2 space-y-0">
                      <FormControl>
                ...
                <FormField control={form.control} name="physicalDamage" render={({ field }) => (<FormItem className="flex items-center space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={(checked) => {
                field.onChange(checked);
                if (checked) {
                ...
                <FormField control={form.control} name="importantFiles" render={({ field }) => (<FormItem className="flex items-center space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange}/>
                      </FormControl>
                ...
                <FormField control={form.control} name="noPower" render={({ field }) => (<FormItem className="flex items-center space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange}/>
                      </FormControl>
                ...
                <FormField control={form.control} name="repairHistory" render={({ field }) => (<FormItem className="flex items-center space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange}/>
                      </FormControl>
                ...
                {!isPublic && (<FormField control={form.control} name="enablePhotoAnnotation" render={({ field }) => (<FormItem className="flex items-center space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange}/>
                      </FormControl>
              ...
              {!isPublic && form.watch("enablePhotoAnnotation") && (<div className="mt-4 space-y-4">
                  <FormField control={form.control} name="annotationDeviceType" render={({ field }) => (<FormItem>
                  ...
                  {form.watch("annotationDeviceType") && (<>
                      <div>
                        <DeviceAnnotationCanvas deviceType={form.watch("annotationDeviceType") || ""} onSave={(dataUrl) => {
                    setAnnotationImageUrl(dataUrl);
                }}/>
                        {annotationImageUrl && (<p className="text-sm text-green-600 mt-2">✓ Annotation saved</p>)}
                      </div>

                      <FormField control={form.control} name="annotationNotes" render={({ field }) => (<FormItem>
                            <FormLabel>Additional Notes (Optional):</FormLabel>
                            <FormControl>
                              <Textarea {...field} placeholder="Add any additional notes about the device condition or damages..." rows={3}/>
                            </FormControl>
                            <FormMessage />
                          </FormItem>)}/>
                    </>)}
            ...
            {!isPublic && (<div className="grid md:grid-cols-2 gap-4">
              <FormField control={form.control} name="estimatedCost" render={({ field }) => (<FormItem>
                    <FormLabel>Estimated Cost (optional):</FormLabel>
                    <FormControl>
                      <Input type="number" placeholder="PHP" value={field.value || ""} onChange={(e) => field.onChange(parseFloat(e.target.value) || 0)}/>
                    </FormControl>
                    <FormMessage />
                  </FormItem>)}/>

              <FormField control={form.control} name="timeFrame" render={({ field }) => (<FormItem>
            ...
            <div>
              ...
              <div className="space-y-3">
                <FormField control={form.control} name="ack1" render={({ field }) => (<FormItem className="flex items-start space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange} disabled={!termsRead} className={!termsRead ? "opacity-50" : ""}/>
                      </FormControl>
                      <div className="flex-1">
                        <FormLabel className="!mt-0 text-sm">
                          I have read and understood the{" "}
                          <button type="button" className="text-blue-600 underline hover:text-blue-800" onClick={() => setTermsModalOpen(true)}>
                ...
                <FormField control={form.control} name="ack2" render={({ field }) => (<FormItem className="flex items-start space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange}/>
                      </FormControl>
                      <div className="flex-1">
                ...
                <FormField control={form.control} name="ack3" render={({ field }) => (<FormItem className="flex items-start space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange}/>
                      </FormControl>
                      <div className="flex-1">
                ...
                {!isPublic && (<FormField control={form.control} name="autoApproveDiagnosis" render={({ field }) => (<FormItem className="flex items-start space-x-2 space-y-0 rounded-lg border border-primary/30 bg-primary/5 p-3">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange}/>
                      </FormControl>
                      <div className="flex-1">
                ...
                {!isPublic && (<FormField control={form.control} name="physicalSignature" render={({ field }) => (<FormItem className="flex items-start space-x-2 space-y-0">
                      <FormControl>
                        <Checkbox checked={field.value} onCheckedChange={field.onChange}/>
                      </FormControl>
                      <div className="flex-1">
              ...
              {!isPublic && form.watch("physicalSignature") && (<div className="mt-4">
                  <FormLabel>Client Signature:</FormLabel>
                  <p className="text-sm text-muted-foreground mb-2">
                    Please draw your signature below
                  </p>
                  <SignatureCanvasComponent ref={signatureRef} onSave={(dataUrl) => {
                ...
                toast({
                  ...
                  {signatureUrl && (<p className="text-sm text-green-600 mt-2">✓ Signature captured</p>)}
                </div>)}
            ...
            <div className="flex gap-4">
              ...
              {!isPublic && (<Button type="button" variant="outline" onClick={() => navigate("/admin-portal")}>
                  Cancel
                </Button>)}
            </div>
          </form>
        ...
        <Dialog open={termsModalOpen} onOpenChange={setTermsModalOpen}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-auto bg-muted flex justify-center items-start p-4">
              <img src={termsImage} alt="AC Tech Repair Terms and Conditions" className="max-w-full h-auto rounded-md shadow-sm"/>
            </div>
    ...
    return isPublic ? (<div className="min-h-screen w-full bg-background">
      {kioskCode ? (<div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-background px-6 text-center">
          ...
          {kioskQr && (<div className="mt-6 flex flex-col items-center">
              <img src={kioskQr} alt={`QR code to view the live queue for ${kioskCode}`} className="h-44 w-44 rounded-2xl border bg-white p-2 md:h-52 md:w-52"/>
    ...
    </div>) : (<DashboardLayout>{content}</DashboardLayout>);
};
