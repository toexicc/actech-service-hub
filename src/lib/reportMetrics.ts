...
export interface ActorOutput {
  ...
  handedOver: number;
  /** Invoice-receipt / warranty card documents generated by this person. */
  documents: number;
...
export const buildActorOutput = (
  ...
  logs.forEach((l) => {
    if (
      (!l.to && l.event !== "payment" && l.event !== "release" && l.event !== "document") ||
      !isPerson(l.actor)
    )
      return;
    const id = String(l.serviceId || "").trim();
